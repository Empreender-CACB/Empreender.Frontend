...
import { ExclaimationTriangleIcon, FolderIcon, LifebuoyIcon } from '@heroicons/react/24/outline'
              ...
              <Combobox onChange={(item) => (window.location = item.url)}>
                ...
                {(filteredProjects.length > 0 || filteredUsers.length > 0) && (
                  ...
                  >
                    {filteredProjects.length > 0 && (
                      <li>
                        <h2 className="text-xs font-semibold text-gray-900">Ações</h2>
                ...
                {rawQuery === '?' && (
                  <div className="py-14 px-6 text-center text-sm sm:px-14">
                    ...
                    Use esta ferramenta para pesquisar recursos em toda a nossa plataforma. Você também pode
                       usar os modificadores de pesquisa encontrados no rodapé abaixo para comandos de administrador.
                    </p>
                  </div>
                )}
                ...
                {query !== '' && rawQuery !== '?' && filteredProjects.length === 0 && filteredUsers.length === 0 && (
                  <div className="py-14 px-6 text-center text-sm sm:px-14">
                    <ExclaimationTriangleIcon className="mx-auto h-6 w-6 text-gray-400" aria-hidden="true" />
                    <p className="mt-4 font-semibold text-gray-900">Sem resultados</p>
                    <p className="mt-2 text-gray-500">Não encontramos nada com esse termo. Por favor, tente novamente.</p>
                  </div>
                )}
                ...
                <div className="flex flex-wrap items-center bg-gray-50 py-2.5 px-4 text-xs text-gray-700">
                  ...
                  <span className="hidden sm:inline">para acessar projetos,</span>
                  <kbd
                    className={classNames(
                      'mx-1 flex h-5 w-5 items-center justify-center rounded border bg-white font-semibold sm:mx-2',
                      rawQuery.startsWith('>') ? 'border-indigo-600 text-indigo-600' : 'border-gray-400 text-gray-900'
